...
export default function Programmers() {
  ...
  let [programmers] = useState(listOfAwesome);
  let [programmerID, setProgrammerID] = useState(null)
  ...
  const getNameOfFeatured = () => {
    ...
    return programmers.find((obj) => obj.id == programmerID).name;
      ...
      <div id='featured' style={style}>
        {
          ...
          programmerID ? `🎉 Let's celebrate ${getNameOfFeatured()}! 🥳` : 'Pick an awesome programmer'
        }
      </div>
    </div>
  );
